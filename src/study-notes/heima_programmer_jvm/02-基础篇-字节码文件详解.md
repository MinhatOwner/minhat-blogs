...
![Java虚拟机主要分为以下几个组成部分](/study-notes/heima_programmer_jvm/1781713889420.png)

- 类加载子系统：核心组件类加载器，负责将字节码文件中的内容加载到内存中。
- 运行时数据区：JVM管理的内存，创建出来的对象、类的信息等等内容都会放在这块区域中。
- 执行引擎：包含了即时编译器、解释器、垃圾回收器，执行引擎使用解释器将字节码指令解释成机器码，使用即时编译器优化性能，使用垃圾回收器回收不再使用的对象。
- 本地接口：调用本地使用C/C++编译好的方法，本地方法在Java中声明时，都会带上native关键字，如下图所示。
![本地接口](/study-notes/heima_programmer_jvm/1781714079620.png)
...
![NotePad++使用十六进制插件查看class文件](/study-notes/heima_programmer_jvm/1781848760210.png)
...
![使用 jclasslib工具查看字节码文件](/study-notes/heima_programmer_jvm/1781848863705.png)
...
![基本信息包含了jclasslib中能看到的两块内容](/study-notes/heima_programmer_jvm/1781849047403.png)
...
![主版本号52就是JDK8](/study-notes/heima_programmer_jvm/1781849266503.png)

版本号的作用主要是判断当前字节码的版本和运行时的JDK是否兼容。如果使用较低版本的JDK去运行较高版本JDK的字节码文件，无法使用会显示如下错误：

版本号的作用主要是判断当前字节码的版本和运行时的JDK是否兼容。如果使用较低版本的JDK去运行较高版本JDK的字节码文件，无法使用会显示如下错误：

![较低版本的JDK去运行较高版本JDK的字节码文件](/study-notes/heima_programmer_jvm/1781849325704.png)
...
![其他基础信息](/study-notes/heima_programmer_jvm/1781849416604.png)

### 2.2.2、常量池

字节码文件中常量池的作用：避免相同的内容重复定义，节省空间。如下图，常量池中定义了一个字符串，字符串的字面量值为123。

![常量池](/study-notes/heima_programmer_jvm/1781849490503.png)
...
![常量池2](/study-notes/heima_programmer_jvm/1781849567103.png)
...
![字段](/study-notes/heima_programmer_jvm/1781849623202.png)

### 2.2.4、方法

字节码中的方法区域是存放**字节码指令**的核心位置，字节码指令的内容存放在方法的Code属性中。

![方法](/study-notes/heima_programmer_jvm/1781849672303.png)
...
![属性](/study-notes/heima_programmer_jvm/1781968196458.png)
...
![javap](/study-notes/heima_programmer_jvm/1782010662121.png)
...
![源代码编译后的字节码文件内容](/study-notes/heima_programmer_jvm/1782010907122.png)
...
3. 输入需要Arthas监控的进程id。
    ![输入需要Arthas](/study-notes/heima_programmer_jvm/1782011726116.png)
4. 输入命令即可使用。
...
![类的生命周期](/study-notes/heima_programmer_jvm/1782220811420.png)
...
![加载阶段](/study-notes/heima_programmer_jvm/1782027114215.png)
...
![加载阶段](/study-notes/heima_programmer_jvm/1782027200128.png)
...
![验证](/study-notes/heima_programmer_jvm/1782027406315.png)
...
![验证](/study-notes/heima_programmer_jvm/1782027477816.png)
...
![堆区](/study-notes/heima_programmer_jvm/1782039436315.png)
...
![常量池](/study-notes/heima_programmer_jvm/1782039518815.png)

### 3.2.3、解析

解析阶段主要是==将常量池中的符号引用替换为直接引用==，符号引用就是在字节码文件中使用编号来访问常量池中的内容。

![解析-符号引用](/study-notes/heima_programmer_jvm/1782039579416.png)

直接引用不在使用编号，而是使用内存中地址进行访问具体的数据。

![解析-直接引用](/study-notes/heima_programmer_jvm/1782039644916.png)
...
![解析-初始化阶段](/study-notes/heima_programmer_jvm/1782088499961.png)
...
![解析-初始化阶段](/study-notes/heima_programmer_jvm/1782088568961.png)

2、putstatic指令会将操作数栈上的数弹出来，并放入堆中静态变量的位置，字节码指令中#2指向了常量池中的静态变量value，在解析阶段会被替换成变量的地址。

![解析-初始化阶段](/study-notes/heima_programmer_jvm/1782088611463.png)
...
![解析-初始化阶段](/study-notes/heima_programmer_jvm/1782088668961.png)
...
![解析-初始化阶段](/study-notes/heima_programmer_jvm/1782089051750.png)

指令会输出D

2. 执行main方法的字节码指令。

![解析-初始化阶段](/study-notes/heima_programmer_jvm/1782089084250.png)

指令会输出A

3. 创建两个对象，会执行两次对象初始化的指令。

![解析-初始化阶段](/study-notes/heima_programmer_jvm/1782089118951.png)
...
![类加载器-什么是类加载器](/study-notes/heima_programmer_jvm/1782131941404.png)
...
![类加载器-类加载器的分类](/study-notes/heima_programmer_jvm/1782133612269.png)

类加载器的详细信息可以通过Arthas的classloader命令查看：

> `classloader`- 查看 classloader 的继承树，urls，类加载信息，使用 classloader 去 getResource

![类加载器-类加载器的分类](/study-notes/heima_programmer_jvm/1782133697769.png)
...
![类加载器-启动类加载器](/study-notes/heima_programmer_jvm/1782133697769.png)
...
![类加载器-用户扩展基础jar包](/study-notes/heima_programmer_jvm/1782133991969.png)
...
![类加载器-用户扩展基础jar包](/study-notes/heima_programmer_jvm/1782134063069.png)
...
![类加载器-扩展类加载器](/study-notes/heima_programmer_jvm/1782134141869.png)
...
![类加载器-扩展类加载器](/study-notes/heima_programmer_jvm/1782134207569.png)
...
![类加载器-扩展类加载器](/study-notes/heima_programmer_jvm/1782134334868.png)
...
![类加载器-应用程序加载器](/study-notes/heima_programmer_jvm/1782134450970.png)

这两个类均由应用程序类加载器加载。

类加载器的加载路径可以通过classloader –c hash值 查看：

![类加载器-应用程序加载器](/study-notes/heima_programmer_jvm/1782134497968.png)

# 5、双亲委派机制

双亲委派机制指的是：当一个类加载器接收到加载类的任务时，会==自底向上查找是否加载过， 再由顶向下进行加载==。

![双亲委派机制](/study-notes/heima_programmer_jvm/1782139850968.png)

详细流程：

每个类加载器都有一个父类加载器。父类加载器的关系如下，启动类加载器没有父类加载器：

![双亲委派机制](/study-notes/heima_programmer_jvm/1782139937468.png)
...
![案例1](/study-notes/heima_programmer_jvm/1782643229888.png)

2、扩展类加载器也没加载过，交给他的父类加载器 - 启动类加载器。

![案例1](/study-notes/heima_programmer_jvm/1782643284686.png)

3、启动类加载器发现已经加载过，直接返回。

![案例1](/study-notes/heima_programmer_jvm/1782643326185.png)

## 案例2：

B类在扩展类加载器加载路径中，同样应用程序类加载器接到了加载任务，按照案例1中的方式一层一层向上查找，发现都没有加载过。那么启动类加载器会首先尝试加载。它发现这类不在它的加载目录中，向下传递给扩展类加载器。

![案例2](/study-notes/heima_programmer_jvm/1782643376786.png)

扩展类加载器发现这个类在它加载路径中，加载成功并返回。

![案例2](/study-notes/heima_programmer_jvm/1782643408586.png)

如果第二次再接收到加载任务，同样地向上查找。扩展类加载器发现已经加载过，就可以返回了。

![案例2](/study-notes/heima_programmer_jvm/1782643437185.png)
...
![如何指定加载类的类加载器](/study-notes/heima_programmer_jvm/1782643539391.png)
